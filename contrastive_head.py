...
class Encoder(torch.nn.Module):
    def __init__(self, input_shape=(32,32), num_feature=64, out_size=64):
        ...
        self.resize1 = F.interpolate(input, size=32, scale_factor=None, mode='nearest')
        #self.dim = torch.movedim(qq, 2, 1)
        self.resize1 = F.interpolate(qq, size=32, scale_factor=None, mode='nearest')
        #self.dim2 = torch.movedim(qq, 2, 1)
    ...
    def forward(self, x):
        ...
        x = F.movedim(self.resize1(x), 2, 1)
        x = F.movedim(self.resize1(x), 2, 1)
        E = F.relu(self.c0(x))
